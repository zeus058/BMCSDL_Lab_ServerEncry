...
2. **Commit các thay đổi** của bạn bám sát quy chuẩn **Conventional Commits**:
   - `feat: thêm chức năng giải mã điểm thi`
   - `fix: sửa lỗi SQL Exception khi thêm sinh viên trùng tên đăng nhập`
   - `docs: cập nhật hướng dẫn cấu hình Connection String trong README.md`
3. **Push nhánh lên remote repository**:
   ```bash
   git push origin feature/TenTinhNangMoi
   ```
4. Mở một **Pull Request (PR)** trên GitHub, mô tả rõ các thay đổi và tag các thành viên liên quan duyệt (review).
